...
function play(){
    ...
    timerInterval = setInterval(() => {
        ...
        let elapsed = ((now - startTime) / 1000).toFixed(2);
...
function makeGuess(){
    ...
    if(userGuess > answer){
        ...
        // store time for averages
        timeArr.push(Number(timeTaken));
        if (fastestTime === null || timeTaken < fastestTime) fastestTime = timeTaken;
        updateTimeStats();
...
function reset(){
    ...
    document.getElementById("gameClock").textContent = "Elapsed time: 0.00 seconds"; // reset timer display
    for(let i=0; i< levelArr.length; i++){
...
// ===== NEW FUNCTION: update time stats =====
